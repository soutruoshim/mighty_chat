...
class LastMessageContainer extends StatelessWidget {
  ...
  Widget typeWidget(ChatMessageModel message) {
    ...
    switch (type) {
      case TEXT:
        return Text("${message.message.validate()}", maxLines: 1, overflow: TextOverflow.ellipsis, style: secondaryTextStyle(size: 16));
      case IMAGE:
        return Row(
          children: [Icon(Icons.photo_sharp, size: 16), 6.width, Text('Image', style: secondaryTextStyle(size: 16))],
        );
      case VIDEO:
        return Row(
          children: [Icon(Icons.videocam_outlined, size: 16), 6.width, Text('Video', style: secondaryTextStyle(size: 16))],
        );
      case AUDIO:
        return Row(
          children: [Icon(Icons.audiotrack, size: 16), 6.width, Text('Audio', style: secondaryTextStyle(size: 16))],
        );
      default:
        return Container();
    }
